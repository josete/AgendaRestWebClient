...
public class ModificarPersonaServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet ModificarPersonaServlet</title>");            
            out.println("</head>");
            out.println("<body>");
             for (PersonaObjetoConId pid : aid.getPersonas()) {
                out.println("<h3>"+pid.getId() + ". " + pid.getNombre()+"</h3>");
            }
            out.println("<h3>Selecciona la persona que quieres modificar</h3>");
            out.println("<form method='POST' action='/AgendaRestWebClient/ModificarPersona'><input type='text' name='nPersona'><input type='submit' value='Modificar Persona'></form>");            
            out.println("</body>");
            out.println("</html>");
        }
    }
